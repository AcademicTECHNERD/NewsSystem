...
//@RestController结合了 @Controller 和 @ResponseBody 两个注解的功能
//作用是让控制器类中的每个方法都默认返回 JSON、XML 或自定义对象，而不是传统的视图页面
//相比于 @Controller，不需要为每个方法都添加 @ResponseBody 来指明返回的对象应该作为响应体（response body），因此省去了手动配置。
@RestController
//@RequestMapping: 用于指定请求路径和 HTTP 方法。
@RequestMapping("/user")
@Validated
...
//    @GetMapping / @PostMapping: 简化了 GET 和 POST 请求的映射。
